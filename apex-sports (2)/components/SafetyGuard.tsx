...
const SafetyGuard: React.FC<SafetyGuardProps> = ({ athlete, children }) => {
    // Normalization check (handle case-insensitive yes/true)
    const consent = athlete.parentConsent?.toString().toLowerCase().trim();
    const hasConsent = consent === 'yes' || consent === 'true';

    // Age Check override? (User only specified "If Parent Consent is NOT Yes")
    // We strictly adhere to that rule.
