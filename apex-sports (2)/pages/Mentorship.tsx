...
import { Brain, Target, Compass, BookOpen, Calendar, UserPlus, ClipboardList, Activity, Lock } from 'lucide-react';
import { LINKS } from '../constants';
import PricingCard from '../components/PricingCard';

// Interactive Components
import SpatQuestionnaire from '../components/Mentorship/SpatQuestionnaire';
import RoutineEditor from '../components/Mentorship/RoutineEditor';
import ResourceLibrary from '../components/Mentorship/ResourceLibrary';

const Mentorship: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'assessment' | 'routines' | 'library'>('overview');

  const tabs = [
    { id: 'overview', label: 'Program Overview', icon: BookOpen },
    { id: 'assessment', label: 'SPAT Assessment', icon: Activity },
    { id: 'routines', label: 'Elite Routines', icon: Calendar },
    { id: 'library', label: 'Resource Library', icon: Brain },
  ];

  // Original Pricing Content for Overview Tab
  const packages = [
    {
  ...
  return (
    <div className="min-h-screen bg-black pt-20">
      ...
      {/* Tab Navigation */}
      <div className="sticky top-24 z-30 bg-black/80 backdrop-blur-md border-b border-neutral-800">
          ...
          <div className="flex gap-8 min-w-max">
            {tabs.map((tab) => (
              <button
                ...
                className={`group flex items-center gap-2 py-6 border-b-2 transition-all ${activeTab === tab.id
      ...
      {/* Content Area */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 min-h-[600px]">
