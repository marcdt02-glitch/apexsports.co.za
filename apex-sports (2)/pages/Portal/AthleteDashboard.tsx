...
const AthleteDashboard: React.FC = () => {
    ...
    // ACWR & Load Data
    const acwrValue = analysis.performance.acwr;
    const acwrColor = acwrValue > 1.5 || acwrValue < 0.8 ? '#ef4444' : '#22c55e';
    const recentSessions = analysis.performance.sessions.slice(0, 7).reverse();
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-8">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="text-xl font-bold text-white">Primary Outcome Goal</h3>
                    </div>
                    {goals.isEditing ? (
                        <textarea
                            value={goals.season}
                ...
                <div className="space-y-4">
                    {['process1', 'process2', 'process3'].map((key, i) => (
                        <div key={key} className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 flex flex-col justify-center">
                            <h4 className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-2">Process Goal {i + 1}</h4>
                            {goals.isEditing ? (
                                <input
                                    type="text"
    ...
    const LibraryView = () => (
        <div className="animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[
                    { title: "Mentorship Handbook", type: "Psychology", desc: "Goal setting and mental framing strategies for high performance." },
                    { title: "S&C Fundamentals", type: "Physiology", desc: "Understanding energy systems and load management." },
                    { title: "Reviewing Game Tape", type: "Tactical", desc: "A guide to analyzing your own performance on video." },
                    { title: "Rest & Recovery", type: "Wellness", desc: "Sleep hygiene and active recovery protocols." }
                ].map((item, i) => (
